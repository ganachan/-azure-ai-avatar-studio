...
# Language configurations with Binaka avatar
LANGUAGE_CONFIG = {
    "English": {
        "code": "en-US",
        "voice": "en-US-AvaMultilingualNeural",
        "avatar_character": "Binaka-half",
        "flag": "🇺🇸",
        "native_name": "English"
    },
    "Spanish": {
...
# Title
st.title("Elevate Your Sales Pitch: Create Personalized AI-Driven Videos with Binaka Avatar")
st.write("Create and download videos using the custom Binaka avatar with built-in voice capabilities and neural voices.")

# Avatar information display
st.markdown("""
<div class="avatar-info">
    <strong>🤖 Avatar Information:</strong><br>
    <strong>Character:</strong> Binaka-half (Custom Avatar)<br>
...
with st.sidebar:
    ...
    <div class="language-card">
        ...
        <strong>Native Name:</strong> {lang_config['native_name']}<br>
        <strong>Avatar:</strong> {lang_config['avatar_character']}
    </div>
    """, unsafe_allow_html=True)
    
    # Avatar settings
    st.header("Avatar Settings")
    st.info("Using Binaka-half custom avatar with built-in voice capabilities")
    
    # Video format options
    video_format = st.selectbox("Video Format", ["mp4", "webm"], index=0)
    video_codec = st.selectbox("Video Codec", ["h264", "hevc"], index=0)
    background_color = st.color_picker("Background Color", "#FFFFFF")

# Default text generation
if username and industry_vertical and customer_name:
    default_input_text = (
        f"Welcome {customer_name}! I'm Binaka, your AI partner from the Global Black Belt AI Team at Microsoft. "
    ...
    default_input_text = "Hi, I'm Binaka, your AI partner from the Global Black Belt AI Team at Microsoft."
...
def check_existing_files(username, industry_vertical, customer_name, file_type, language):
    if not blob_connected:
        return 1
    file_prefix = f"{username}_{industry_vertical}_{customer_name}_{language}_Binaka_{file_type}"
    try:
...
def submit_synthesis(job_id: str, text_content: str, language_config: dict, video_format: str, video_codec: str, bg_color: str):
    """Submit synthesis job with Binaka avatar and built-in voice settings"""
    ...
    payload = {
        ...
        "avatarConfig": {
            "customized": True,  # Set to True for custom Binaka avatar
            "talkingAvatarCharacter": language_config['avatar_character'],
            "videoFormat": video_format,
            "videoCodec": video_codec,
            "subtitleType": "soft_embedded",
            "backgroundColor": bg_color,
            "useBuiltInVoice": True  # Enable built-in voice for avatar synchronization
        }
    }
...
def generate_filename(username, industry_vertical, customer_name, language, count, extension, file_type):
    return f"{username}_{industry_vertical}_{customer_name}_{language}_Binaka_{file_type}{count}.{extension}"
...
with st.expander("Current Settings", expanded=False):
    ...
    st.write(f"**Avatar:** {lang_config['avatar_character']} (Custom Avatar)")
    st.write(f"**Video Format:** {video_format}")
    st.write(f"**Video Codec:** {video_codec}")
...
if st.button(f"Generate {selected_language} Binaka Avatar Video", type="primary", disabled=(char_count > max_chars)):
    if not username or not input_text or not customer_name:
        ...
        st.info(f"Creating {selected_language} Binaka avatar video with {lang_config['voice']} voice and built-in voice synchronization...")
        
        # Submit synthesis job
        submitted_job_id = submit_synthesis(job_id, final_text, lang_config, video_format, video_codec, background_color)
        if submitted_job_id:
            ...
            with st.spinner(f"Processing {selected_language} Binaka avatar video..."):
                max_attempts = 60
                attempts = 0
                video_content = None
                ...
                while attempts < max_attempts:
                    ...
                    if download_url and response_data:
                        st.success(f"{selected_language} Binaka avatar video generated successfully!")
                        progress_bar.progress(1.0)
                        status_text.text("Completed!")

                        video_name = generate_filename(username, industry_vertical, customer_name, selected_language, recording_count, video_format, "recordings")
                        ...
                        try:
                            ...
                            with col1:
                                if video_content:
                                    st.download_button(
                                        label=f"Download {selected_language} Binaka Video",
                                        data=video_content,
...
# Video history
st.subheader("Multilingual Binaka Avatar Video History")
if st.session_state['video_history']:
    for i, video in enumerate(reversed(st.session_state['video_history'])):
        with st.expander(f"🤖 {video['flag']} {video['language']} Binaka Video {len(st.session_state['video_history']) - i}: {video['name']}", expanded=False):
            col1, col2 = st.columns([2, 1])
            
            with col1:
                st.write(f"**Language:** {video['flag']} {video['language']}")
                st.write(f"**Avatar:** {video.get('avatar', 'Binaka-half')}")
    ...
    st.info("No Binaka avatar videos generated in this session yet.")
...
<div style="text-align: center; color: #666; padding: 20px;">
    <p>Powered by Azure AI Services | Binaka Custom Avatar with Built-in Voice Technology</p>
</div>
""", unsafe_allow_html=True)