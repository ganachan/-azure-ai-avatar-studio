...
def calculate_tokens(text):
    """Calculate tokens based on words/4 formula"""
    words = len(text.split())
    tokens = max(1, words // 4)  # Ensure at least 1 token
    return tokens, words
...
def submit_synthesis(job_id: str, text_content: str, lang_config: dict, avatar_config: dict, video_format: str, video_codec: str, bg_color: str):
...
if st.button(f"Generate {avatar_config['display_name']} {selected_language} Video", type="primary", disabled=(char_count > max_chars)):
    if not username or not input_text or not customer_name:
        ...
        submitted_job_id = submit_synthesis(job_id, final_text, lang_config, avatar_config, video_format, video_codec, background_color)
        if submitted_job_id:
                ...
                while attempts < max_attempts:
                    ...
                    if download_url and response_data:
                        ...
                        try:
                            ...
                            with col1:
                                if video_content:
                                    video_name = f"{username}_{customer_name}_{avatar_config['display_name']}_{selected_language}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.{video_format}"
                                    st.download_button(
...
<div style="text-align: center; color: #666; padding: 20px;">
    ...
    <p><strong>Token Calculation:</strong> Words ÷ 4 | <strong>Engagement:</strong> Each video = 1 engagement</p>
</div>
""", unsafe_allow_html=True)