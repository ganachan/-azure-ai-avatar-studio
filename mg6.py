...
class AvatarService:
    ...
    def submit_avatar_synthesis(self, job_id: str, text: str, customer_name: str = "Valued Customer"):
        ...
        # Create personalized greeting with case summary
        personalized_text = f"""
        Hello {customer_name}, I'm Maria, your AI support specialist from Microsoft.
        
        I've reviewed your case and our technical team has processed it through our multi-agent system.
        
        {text}
        
        We're committed to your success and will continue monitoring to ensure everything works perfectly.
        ...
        payload = {
            ...
            "inputKind": "plainText",
            "inputs": [{"content": personalized_text}],
            "avatarConfig": {
                "customized": False,
...
class MultiAgentProcessor:
    ...
    async def process_case_with_rag(self, case_data, progress_container):
        """Enhanced case processing with RAG integration and resolution summary"""
        if not self.agents:
...
def main():
    ...
    with st.sidebar:
        ...
        if knowledge_service.search_client:
            st.markdown('<div class="status-card success-card">🔍 RAG System: Connected</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="status-card warning-card">🔍 RAG System: Not configured</div>', unsafe_allow_html=True)
        
        if cosmos_service.connected:
            st.markdown('<div class="status-card success-card">💾 Database: Connected</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="status-card warning-card">💾 Database: Not configured</div>', unsafe_allow_html=True)
        
        if avatar_service.connected:
            st.markdown('<div class="status-card success-card">📹 Avatar: Connected</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="status-card warning-card">📹 Avatar: Not configured</div>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        if st.button("🚀 Process Latest Case with AI Agents", type="primary"):
            ...
            if latest_case:
                ...
                st.warning("⚠️ No cases found. Please save a case first.")
    ...
    with col2:
        ...
        if st.session_state.get('resolution_summary'):
            ...
            <div class="status-card success-card">
                ✅ Resolution summary ready for avatar generation
            </div>
            """, unsafe_allow_html=True)
